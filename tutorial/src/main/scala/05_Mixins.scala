...
object Mixins_ extends App {

  println(">>>>>>>>>>>>>>>>>>>>>>>>>>");
  println(">>>>>>>>>>>>>>>>>>>>>>>>>>");
  println("CLASS COMPOSITION WITH MIXIN !");
  println(">>>>>>>>>>>>>>>>>>>>>>>>>>");;
  println(">>>>>>>>>>>>>>>>>>>>>>>>>>");;
...
// Next, we’ll implement a concrete class (all abstract members T, hasNext, and next have implementations):
  ...
  val miaStinga = new StringIterator("Dajeee Pasuqale")

  while (miaStinga.hasNext) {
    println(miaStinga.next())
  }
  ...
  //  f: T => Unit on the next element (next()) as long as there are further elements (while (hasNext)). Because RichIterator is a trait, it doesn’t need to implement the abstract members of AbsIterator.

  // We would like to combine the functionality of StringIterator and RichIterator into a single class.
  println("==================");
  println("RichStringIter : ");
  println("==================");
